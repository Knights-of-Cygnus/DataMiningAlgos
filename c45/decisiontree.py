...
class DTree:
    def __init__(self, label: T, threshold: Optional[Numeric] = None):
        self.label = label
...
class C45Algo:
    ...
    def generate_tree(self, current_data: List[list], current_attribute: List[int]):
        ...
        gain, attribute, threshold, split = max(self.try_split(current_data, current_attribute), key=itemgetter(0))
        remaining = [att for att in current_attribute if att != attribute]
        node = DTree(attribute, threshold)
        node.children = [self.generate_tree(subset, remaining) for subset in split]
        return node
